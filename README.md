...
在NapCat中添加 **`WebSocket Client`** ，填入地址 `ws://{host}:{port}/ws/` ，`host` 和 `port` 为上一步填写的 **WebSocket Server地址和端口**，例如默认地址`ws://127.0.0.1:12345/ws/`。
...
运行 `main.py`，等待bot连接，成功建立连接后开始下载图片。
